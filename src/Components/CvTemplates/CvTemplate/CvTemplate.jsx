    ...
    <Document>
      ...
      <Page wrap size='A4' style={styles.body}>
          ...
          <View>
            <View style={styles.intro__name_job}>
              <Text style={styles.intro__name}>
                {cvData.name === '' ? 'Nazar' : cvData.name} {cvData.surname === '' ? 'Archakov' : cvData.surname}
              </Text>
              <Text style={styles.intro__job}>{cvData.title === '' ? 'Frontend-developer' : cvData.title}</Text>
            </View>
            <View style={styles.intro__info}>
              <Text style={styles.intro__info_item}>{cvData.phone === '' ? '80291844650' : cvData.phone}</Text>
              <Text style={styles.intro__info_item}>{cvData.email === '' ? 'Email@mail.ru' : cvData.email}</Text>
            </View>
          </View>
        </View>
        <View style={styles.about}>
          ...
          >
            {cvData.summary === '' ? 'Lorem ipsum dolor sit amet consectetur, adipisicing elit. Tempore suscipit neque earum repellat laudantium dolor. Lorem ipsum, dolor sit amet consectetur adipisicing elit. Praesentium deserunt cum qui veniam alias laboriosam nesciunt.' : cvData.summary}
          </Text>
        </View>
        <View style={styles.edu}>
          <Text style={styles.edu__title}>Experience</Text>
          <View style={styles.edu__items}>
            {cvData.employmentHistory.length === 0 ? <DefaultExpirienceItem/> :
        ...
        <View style={styles.exp}>
          ...
          <View style={styles.exp__items}>
            {cvData.educationHistory.length === 0 ? <DefaultEducationItem/> :
        ...
        <View style={styles.skills}>
          ...
          <View style={styles.skills__list}>
            {cvData.skills.length === 0 ? DefaultSkillAray.map((elem) =>(
